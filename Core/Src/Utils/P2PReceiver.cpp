...
namespace P2PPackage {
    ...
    P2PReceiver::P2PReceiver() {
        ...
        for (uint8_t i = 0; i < UserSetting::P2P_PACKET_SIZE; ++i) {
            receive_buffer[i] = 0;
        }
    }
    ...
    uint8_t P2PReceiver::getDataLen() const {
        return static_cast<uint8_t>(UserSetting::P2P_PACKET_SIZE);
    }
    ...
    void P2PReceiver::Process(const uint8_t* buf, uint8_t len, FlightManager& manager) {
        ...
        // Ensure fixed length
        if (len != UserSetting::P2P_PACKET_SIZE){
            return;
        }

        P2PPacketDecoder decoder;
        if (decoder.SetData(buf, len) != PacketError::SUCCESS) {
            return;
        }
