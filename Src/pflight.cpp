...
SBUS sbus;
PID pitch(0.7, 0.0, 0.0);
PID roll(0.7, 0.0, 0.0);
PID yaw(0.3, 0.0, 0.0);
...
void init_sbus(){
	...
	HAL_UART_Receive_DMA(&huart5, RawSbus, 25);
}
...
void pid_calc(){
	...
	pitch.SetData(control.target[0], data.angle[0], 0.02);
	roll.SetData(control.target[1], data.angle[1], 0.02);
	yaw.SetData(control.target[2], data.angle[2], 0.02);

	control.pid_output[0] = pitch.GetData();
	control.pid_output[1] = roll.GetData();
	control.pid_output[2] = yaw.GetData();
}

void pid_to_pwm(){

	pwm.motor[0] = (2500 * 0.45) + control.throttle + control.pid_output[0] + control.pid_output[1] - control.pid_output[2];
	pwm.motor[1] = (2500 * 0.45) + control.throttle + control.pid_output[0] - control.pid_output[1] + control.pid_output[2];
	pwm.motor[2] = (2500 * 0.45) + control.throttle - control.pid_output[0] + control.pid_output[1] - control.pid_output[2];
	pwm.motor[3] = (2500 * 0.45) + control.throttle - control.pid_output[0] - control.pid_output[1] + control.pid_output[2];
}
...
void idel_pwm(){
	...
	//Servo_idel
	__HAL_TIM_SET_COMPARE(&htim12, TIM_CHANNEL_1, pwm.servo_close);
	__HAL_TIM_SET_COMPARE(&htim12, TIM_CHANNEL_2, pwm.servo_close);
}
...
void generate_pwm(){
	...
	//Servo
	__HAL_TIM_SET_COMPARE(&htim12, TIM_CHANNEL_1, pwm.servo[0]);
	__HAL_TIM_SET_COMPARE(&htim12, TIM_CHANNEL_2, pwm.servo[1]);
}
